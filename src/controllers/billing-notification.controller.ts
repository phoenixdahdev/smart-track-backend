...
export class BillingNotificationController {
  ...
  async triggerAuthThresholds(
    ...
    if (dto.authorization_id) {
      ...
      return { message: 'Auth threshold check completed', data: { checked: 1 } };
    }

    const result = await this.triggerService.checkAllAuthThresholds(orgId);
    return { message: 'Auth threshold check completed', data: result };
  }
}
